...
import countryBoundaries from '../public/data/countries.geojson'
...
function MapFocus({ country }: { country: CountryCode }) {
  const map = useMap()
  useEffect(() => {
    map.flyTo(countryCenters[country], country === 'PRT' ? 5.2 : 5, { duration: 0.7 })
  }, [country, map])
  return null
}
...
function App() {
  ...
  const mapStyle = (feature?: Feature<Geometry, { iso_a3?: CountryCode }>): PathOptions => {
  ...
  const onEachCountry = (
    feature: Feature<Geometry, { iso_a3?: CountryCode; name?: string }>,
    layer: Layer,
  ) => {
    const code = feature.properties?.iso_a3
    const item = ranked.find((country) => country.code === code)
    ...
    layer.on({ click: () => setSelectedCode(code) })
  }
    ...
    <div className="app-shell">
      ...
      <main id="top">
        <section className="hero">
          ...
          <p className="hero__business-summary">
            Built for European sustainability funds, this dashboard identifies where climate-resilient rice investment
            deserves a closer look. Under the default priorities, Italy ranks first of four with a 68.1 score, 19.3
            points ahead of Spain, combining Europe&apos;s largest rice sector in this comparison with high water pressure
            and agricultural water use. The result points fund teams toward climate-smart irrigation, loss reduction,
            and regional grower-support projects while keeping final investment approval subject to local due diligence.
          </p>
        </section>
          ...
          <section className="decision-grid" aria-label="Investment recommendation and priority map">
            <article className="recommendation-card">
              ...
              <div className="sensitivity-note">
                ...
                <p><strong>{leader.totalScore - runnerUp.totalScore >= 8 ? 'Stable lead' : 'Weight-sensitive lead'}</strong><br />{(leader.totalScore - runnerUp.totalScore).toFixed(1)} points ahead under current priorities.</p>
              </div>
            </article>
              ...
              <div className="map-wrap">
                ...
                <MapContainer center={[41, 8]} zoom={4} minZoom={3} maxZoom={8} scrollWheelZoom className="map">
                  ...
                  <GeoJSON
                    ...
                    data={countryBoundaries as GeoJSON.FeatureCollection}
                    style={mapStyle}
                    onEachFeature={onEachCountry}
                  />
                  <MapFocus country={selectedCode} />
                </MapContainer>
                <div className="map-instruction">Click a country to investigate</div>
              </div>
            </article>
          </section>
        ) : (
              ...
              <div className="metric-grid">
                {indicatorOrder.map((key) => (
                  <div className="metric" key={key}>
                    <div className="metric__top"><span>{indicators[key].shortLabel}</span><strong>{Math.round(selected.scores[key])}</strong></div>
                    <div className="metric__track"><i style={{ width: `${selected.scores[key]}%` }} /></div>
                    <div className="metric__bottom"><span>{formatRaw(selected, key)}</span><span>{indicators[key].sourceYear}</span></div>
                  </div>
                ))}
              </div>
      ...
      {methodologyOpen && (
        <div className="modal-backdrop" role="presentation" onMouseDown={() => setMethodologyOpen(false)}>
          <section className="methodology-modal" role="dialog" aria-modal="true" aria-labelledby="methodology-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="methodology-modal__header">
              ...
              <button className="close-button" onClick={() => setMethodologyOpen(false)} aria-label="Close methodology">×</button>
            </div>
            <p className="method-intro">The score compares four countries using dated public-data snapshots. It supports screening, not hydrological or financial forecasting.</p>
